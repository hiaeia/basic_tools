...
def do_guess_start_address(kallsyms, vmlinux): 
    ...
    #默认的启动地址8字节出存放着加载偏移的int
    if kallsyms['arch']==64:
        if _startaddr_from_banner != _startaddr_from_xstext:
            start_addrs.append( 0xffffff8008000000 + INT(8, vmlinux) )
        else:
            if _startaddr_from_banner == 0:
                start_addrs.append( 0xffffff8008000000 + INT(8, vmlinux) )
    ...
    for addr in start_addrs:
        ...
        assert False,"  [!]kernel start address error...内核地址，通过cat /proc/kallsyms |head 猜测出来"
